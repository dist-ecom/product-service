...
async function inspectCacheWithDocker() {
  ...
  try {
    ...
    const otherKeys = keys.filter(
      k => !k.startsWith('product:') && !k.startsWith('products:') && !k.startsWith('search:'),
    );
    ...
    if (productKeys.length > 0) {
      ...
      for (const key of productKeys) {
        const { stdout: ttlOutput } = await execAsync(
          `docker exec shared-redis redis-cli TTL "${key}"`,
        );
    ...
    if (keys.length > 0) {
      ...
      const { stdout: valueOutput } = await execAsync(
        `docker exec shared-redis redis-cli GET "${sampleKey}"`,
      );
      try {
