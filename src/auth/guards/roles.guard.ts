...
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    this.logger.debug(
      `User role: ${userRole}, Required roles: ${normalizedRequiredRoles.join(', ')}`,
    );
    ...
    if (!hasRole) {
      this.logger.warn(
        `Access denied for user ${user.id} with role ${userRole}. Required roles: ${normalizedRequiredRoles.join(', ')}`,
      );
    } else {
